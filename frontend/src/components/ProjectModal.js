...
class ProjectModal extends Component {
  ...
  render() {
    return (
      <Modal size="lg" show={this.props.show} onHide={this.props.handleClose} animation={false}>
          ...
          <Container>
            <Row className="align-items-center">
              ...
              </Col>
              {
                ...
                !this.centralState.user.votes.includes(this.props.project.id) &&
                <Button variant="success" style={{ width: '150px' }} disabled={this.props.disabled || this.centralState.user.votes.length >= 3} onClick={() => vote(this.props.project.id)}>Vote for project!</Button>
                }
              {
                this.centralState.user != null &&
                this.centralState.user.votes.includes(this.props.project.id) &&
                <Button variant="danger" style={{ width: '150px' }} disabled={this.props.disabled} onClick={() => unvote(this.props.project.id)}>Unvote project</Button>
              }
            </Row>
          </Container>
        </Modal.Footer>
